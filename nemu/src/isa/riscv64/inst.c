...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, Reg(dest) = SEXT(Mr(src1 + imm, 1), 32));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, Reg(dest) = SEXT(Mr(src1 + imm, 2), 32));
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", slt    , R, Reg(dest) = src1 < src2);
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, Reg(dest) = SEXT(BITS(src1, 31, 0) >> src2, 32));
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, Reg(dest) = SEXT((sword_t)SEXT(BITS(src1, 31, 0), 32) >> src2, 32));
