...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , I, etrace(false, cpu.pc, csrs("mcause"), csrs("mepc")); s->dnpc = csrs("mepc"); csrs("mstatus") = 0xa00001800;);
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, Reg(dest) = SEXT((sword_t)BITS(src1, 31, 0) % (sword_t)BITS(src2, 31, 0), 32));
  INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw  , R, Reg(dest) = BITS(src1, 31, 0) % BITS(src2, 31, 0));
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, Reg(dest) = SEXT((sword_t)BITS(src1, 31, 0) / (sword_t)BITS(src2, 31, 0), 32));
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, Log(" %lx %ld %ld",s->pc, src1, src2); s->dnpc = (sword_t)src1 >= (sword_t)src2 ? s->pc + imm : s->snpc);
