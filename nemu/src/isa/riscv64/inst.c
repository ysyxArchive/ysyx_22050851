...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 000 ????? 01110 11", subw   , I, R(dest) = SEXT(BITS(src1, 31, 0) - BITS(src2, 31, 0), 32));
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , I, R(dest) = src1 + src2);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->pc = src1 == src2 ? s->pc + imm : s->pc);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->pc = src1 != src2 ? s->pc + imm : s->pc);
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->pc = (sword_t)src1 < (sword_t)src2 ? s->pc + imm : s->pc);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->pc = (sword_t)src1 >= (sword_t)src2 ? s->pc + imm : s->pc);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, s->pc = src1 < src2 ? s->pc + imm : s->pc);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s->pc = src1 >= src2 ? s->pc + imm : s->pc);
