...
class Cache(cellByte: Int = 64, wayCnt: Int = 2, groupSize: Int = 1, addrWidth: Int = 64) extends Module {
  ...
  // when sendReq
  axiIO.AR.bits.addr := Cat(Seq(tag, counter << (tagOffset - log2Up(axiIO.dataWidth / 8))))
  axiIO.AR.bits.id   := 0.U
  axiIO.AR.bits.prot := 0.U
  axiIO.AR.valid     := cacheFSM.is(sendReq)
  // when waitRes
  val mask       = Reverse(Cat(Seq.tabulate(updateTimes)(index => Fill(axiIO.getWidth, UIntToOH(counter)(index)))))
  val maskedData = Fill(updateTimes, axiIO.getWidth.U) & mask
