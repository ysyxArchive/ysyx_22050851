...
class BHMultiplier extends Module {
  ...
  for (i <- 0 until 128) {
    if (i == 0) {
      wallaceLayer3(i).io.bits := Mux(io.mulw, VecInit(addBuffer.map(v => v(i))), sBuffer2(i))
    } else {
      wallaceLayer3(i).io.bits := Mux(io.mulw, VecInit(addBuffer.map(v => v(i))), Cat(cBuffer2(i - 1), sBuffer2(i)))
    }
